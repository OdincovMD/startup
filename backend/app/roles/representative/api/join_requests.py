...
async def _notify_lab_join_created(lab, researcher_full_name: str, request_id: int):
    ...
    if lab.organization_id:
        user_ids = await Orm.get_lab_admin_user_ids_for_organization(lab.organization_id)
    else:
    ...
    for uid in user_ids:
        await Orm.create_notification(uid, "lab_join_request_created", data)
...
async def _notify_org_join_created(org_id: int, lab_name: str, request_id: int):
    ...
    user_ids = await Orm.get_lab_admin_user_ids_for_organization(org_id)
    data = {"request_id": request_id, "lab_name": lab_name}
    for uid in user_ids:
        await Orm.create_notification(uid, "org_join_request_created", data)
...
async def create_lab_join_request(
    ...
    lab = await Orm.get_laboratory_by_public_id(body.lab_public_id.strip())
    if not lab:
    ...
    try:
        req = await Orm.create_lab_join_request(researcher.id, lab.id)
...
async def leave_laboratory(
    ...
    if not researcher:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Профиль исследователя не найден")
    ok = await Orm.leave_laboratory(researcher.id, laboratory_id)
    if not ok:
...
async def create_org_join_request(
    ...
    org = await Orm.get_organization_by_public_id(body.org_public_id.strip())
    if not org:
    ...
    try:
        req = await Orm.create_org_join_request(lab.id, org.id)
...
async def get_my_join_requests(current_user=Depends(get_current_user)):
    ...
    if _is_researcher(current_user):
        researcher = await Orm.get_researcher_by_user(current_user.id)
        if researcher:
            lab_requests = await Orm.get_lab_join_requests_for_researcher(researcher.id)
    if is_lab_representative(current_user):
        org_requests = await Orm.get_org_join_requests_for_researcher_or_lab_rep(current_user.id)
    return {
        "lab": [
            {
                "id": r.id,
...
async def get_org_incoming_lab_requests(current_user=Depends(get_current_user)):
    ...
    if not org:
        return []
    requests = await Orm.get_lab_join_requests_for_org(org.id)
    return [
        {
            "id": r.id,
...
async def get_creator_incoming_lab_requests(current_user=Depends(get_current_user)):
    ...
    if not is_lab_representative(current_user):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Доступно только для представителя лаборатории")
    requests = await Orm.get_lab_join_requests_for_creator(current_user.id)
    return [
        {
            "id": r.id,
            "researcher": {"id": r.researcher.id, "full_name": r.researcher.full_name}
...
async def approve_lab_join_request(
    ...
    req = await Orm.get_lab_join_request_by_id(request_id)
    if not req:
    ...
    updated = await Orm.approve_lab_join_request(request_id)
    if not updated:
...
async def reject_lab_join_request(
    ...
    if not can_reject:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Нет прав на обработку этой заявки")
    updated = await Orm.reject_lab_join_request(request_id)
    if not updated:
...
async def get_org_incoming_org_requests(current_user=Depends(get_current_user)):
    ...
    if not org:
        return []
    requests = await Orm.get_org_join_requests_for_org(org.id)
    return [
        {
            "id": r.id,
            "laboratory": {"id": r.laboratory.id, "name": r.laboratory.name, "public_id": r.laboratory.public_id}
...
async def approve_org_join_request(
    ...
    if not _is_lab_admin(current_user):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Доступно только для представителя организации")
    req = await Orm.get_org_join_request_by_id(request_id)
    if not req:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Заявка не найдена")
    org = await Orm.get_organization_for_user(current_user.id)
    if not org or req.organization_id != org.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Нет прав на обработку этой заявки")
    updated = await Orm.approve_org_join_request(request_id)
    if not updated:
...
async def reject_org_join_request(
    ...
    if not org or req.organization_id != org.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Нет прав на обработку этой заявки")
    updated = await Orm.reject_org_join_request(request_id)
    if not updated:
...
async def leave_organization(
    ...
    if not is_lab_representative(current_user):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Доступно только для представителя лаборатории")
    result = await Orm.leave_organization(request_id, current_user.id)
    if not result:
    ...
    user_ids = await Orm.get_lab_admin_user_ids_for_organization(result["org_id"])
    for uid in user_ids:
        await Orm.create_notification(
            uid,
            "org_join_left",
